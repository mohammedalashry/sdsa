...
export function mapTournamentTeamsList(apiResponse: any) {
  ...
  return tournament.teams.map((t: any) => ({
    team: {
      id: safeNumber(t.id),
      name: safeString(t.team, "Unknown Team"),
      code: "-", // not in source
      country: tournament.season.includes("Saudi") ? "Saudi-Arabia" : "Unknown", // crude check
      founded: 0, // not in source
      national: false, // assumption
      logo: `https://media.api-sports.io/football/teams/${t.id}.png`, // convention
    },
    venue: {
      id: safeNumber(t.stadium?.id),
      name: safeString(t.stadium?.name, "Unknown Venue"),
      address: "-", // not in source
      city: "-", // not in source
      capacity: 0, // not in source
      surface: "-", // not in source
      image: t.stadium?.id
...
export function mapTeamInfo(teamInfoResp: any, entityTeamResp?: any) {
  ...
  const tiTeam = ti?.team ?? {}; // { team_id, name, country, stadium, is_national_team, ... }
  ...
  // EntityTeam (map1)
  const et = entityTeamResp?.root?.object ?? {}; // TEAM { id, name, club:{logo}, country:{name}, stadium:{...}, coach:{...}, ... }
  ...
  const primaryStadium = tiTeam?.stadium || et?.stadium || {};
  const firstMatchStadium = tiMatches.find(Boolean)?.objStadium ?? {};
  ...
  if (et?.coach?.id || et?.coach?.name) {
    rawCoaches.push({
      ...
      dtDOB: et?.coach?.dob || "-",
      nationality: et?.coach?.nationality || "-",
      photo: et?.coach?.photo || "-",
      height: et?.coach?.height || "-",
      weight: et?.coach?.weight || "-",
      birth: {
        date: et?.coach?.dob || "",
  ...
  const dedupedCoaches = uniqBy(rawCoaches, (c: any) => {
    ...
    return id ?? (c?.strCoachNameEn || c?.strCoachNameAr || c?.name || "-");
  });
  ...
  const totalPlayers = players.length ? players.length : 0;
  ...
  const ages: number[] = players
    ...
    .filter((a: number): a is number => typeof a === "number");
  ...
  const clubMarketValue = ti?.clubMarketValue ?? et?.marketValue ?? "-";
...
export function mapStatsToTeam(apiResponse: any, leagueInfo: any) {
  ...
  return {
    ...
    average_team_rating: getStat(["xg"], "-"),
    rank: getStat(["rank"], "-"),
    rank_total: getStat(["rank total"], "-"),
  };
}
...
export function mapTeamFixtures(apiResponse: any, leagueData: any) {
  ...
  function calculatePeriods(timestamp: number | null) {
    if (!timestamp) return { first: 0, second: 0 };
    return {
      first: timestamp,
  ...
  response.matches.forEach((match: any) => {
    ...
    const mappedFixture = {
      fixture: {
        ...
        periods,
        venue: {
          id: match.objStadium?.intID || venue?.stadium_id || "-",
          name:
            match.objStadium?.strStadiumNameEn ||
            match.objStadium?.strStadiumNameAr ||
            venue?.stadium_name ||
            "-",
          city: match.objStadium?.strCity || "-",
        },
        status: matchStatus,
      },
      ...
      league: {
        ...
        logo: league.logo, // "-" unless present in leagueData
        flag: league.flag, // "-" unless present in leagueData
      ...
      score: {
        halftime: { home: "-", away: "-" },
        fulltime: {
          home: match.intHomeTeamScore ?? "-",
          away: match.intAwayTeamScore ?? "-",
        },
        extratime: {
          home: match.boolExtraTime ? match.intHomeTeamScore : "-",
          away: match.boolExtraTime ? match.intAwayTeamScore : "-",
        },
        penalty: {
          home: match.intPenaltyShootoutHomeScore ?? "-",
          away: match.intPenaltyShootoutAwayScore ?? "-",
        },
      },
      tablePosition: { home: "-", away: "-" },
      averageTeamRating: { home: "-", away: "-" },
    };
  ...
  return [
    {
      ...
      players: basePlayers.map((p: any) => {
        ...
        const info = baseId ? infoById.get(baseId) : "-";
        let statsRaw = baseId ? statsById.get(baseId) : "-";
        ...
        const baseFull = p?.nickname || p?.name || "-";
        const infoFull =
          ...
          "-";
        const statsFull =
          statsPayload?.data?.nick_name || statsPayload?.data?.name || "-";
        const fullName = baseFull || infoFull || statsFull;

        const [fn, ...lnRest] = (fullName || "").split(" ").filter(Boolean);
        const finalFirst = p?.name ? p.name.split(" ")[0] : fn || "-";
        ...
        const age =
          birthDate && birthDate instanceof Date && !isNaN(birthDate.getTime())
        ...
        const position =
          p?.position?.primary?.name ?? statsPayload?.data?.primary_position ?? "-";

        const nationalityName = p?.nationality?.name ?? "-";
        ...
        const out = {
          player: {
            ...
            photo: p?.photo ?? infoPlayer?.photo ?? "-",
          },
          statistics: [
            {
              team: {
              ...
              league: {
                ...
                country: tournamentMeta?.organizer?.country?.name ?? "-",
                logo: tournamentMeta?.logo ?? "-",
                flag: tournamentMeta?.flag ?? "-",
        ...
        if (statsPayload) {
          ...
          if (hasStats) {
            ...
            // shirt number
            st0.games.number = st0.games.number ?? infoPlayer?.intShirtNumber ?? "-";
            ...
            const dob = out.player.birth.date ?? infoPlayer?.dtDOB ?? "-";
...
export function mapTeamFormOverview(apiResponse: any) {
  ...
  const toUnix = (dt?: string | "-") => {
    if (!dt) return "-";
    const iso = dt.replace(" ", "T") + "Z";
    const t = Date.parse(iso);
    return Number.isNaN(t) ? "-" : Math.floor(t / 1000);
  };
  ...
  return matches.map((m: any) => {
    ...
    const homeGoals = Number.isFinite(m?.score?.home) ? m.score.home : "-";
    const awayGoals = Number.isFinite(m?.score?.away) ? m.score.away : "-";
    ...
    return {
      ...
      // league === tournament (by name); IDs/logos not present in map1, so "-"
      league: {
        id: "-",
        name: m?.tournament ?? "-", // ← tournament → league.name
        country: "-",
        logo: "-",
        flag: "-",
  ...
  const substitutes = (data.endOfMatchFormation || [])
    ...
    .map((p: any) => ({
      player: {
        ...
        grid: "-", // Substitutes don't need grid positions
      },
    }));
